...
export const getReservationDetail = createAsyncThunk('GET_RESERVATION_DETAILS', async (userId, id) => {
  const response = await axios.get(`https://experthub-production.up.railway.app/api/v1/users/${userId}/appointments/${id}`);
  return response.data;
});
...
export const addReservation = createAsyncThunk('ADD_RESERVATION', async (data) => {
  const user = JSON.parse(localStorage.getItem('expert-current-user'));
  const userId = user.id;
  const response = await axios.post(`https://experthub-production.up.railway.app/api/v1/users/${userId}/appointments`, data);
  return response.data;
});

export const deleteReservation = createAsyncThunk('DELETE_RESERVATION', async (userId, id) => {
  const response = await axios.delete(`https://experthub-production.up.railway.app/api/v1/users/${userId}/appointments/${id}`);
  return response.data;
});
